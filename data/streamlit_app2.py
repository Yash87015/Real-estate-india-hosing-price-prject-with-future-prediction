...
def load_model_and_features():
    """
    Loads the trained ML model and its feature names.
    This function is cached to prevent reloading the model on every rerun.
    """
    # Adjust path if your model files are in a different location relative to streamlit_app.py
    model = joblib.load('data/best_rf_model.joblib')
    features = joblib.load('data/model_features.joblib')
...
def prediction_page():
    ...
    # Load df_ml to get default/min/max values for inputs
    df_eda, df_ml = load_data()
    ...
    # You might want to customize widget types (e.g., st.selectbox for categorical)
    # and provide user-friendly labels/options, which would require saving encoders.
    # For simplicity, we'll use number_input for all features from df_ml's numerical columns.

    # Filter out target/derived columns from model_features if they exist in df_ml
    # Note: 'Growth_Rate', 'Price_in_Lakhs', 'Price_per_SqFt', 'Good_Investment', 'Future_Price_SY' were excluded from X during training.
    # Ensure the list of features here matches the 'X_train.columns' when the model was trained.

    # Example of how to get the columns that were in X for training (excluding targets and derived columns)
    # This list should ideally come from 'model_features' loaded from joblib, but as a fallback/demonstration:
    feature_cols_for_input = [col for col in model_features if col not in ['Price_in_Lakhs', 'Price_per_SqFt', 'Growth_Rate', 'Future_Price_SY', 'Good_Investment', 'ID']]

    # Use st.columns for a nicer layout
    cols = st.columns(3)
    col_idx = 0

    for feature in feature_cols_for_input:
        with cols[col_idx]:
            if feature.startswith('Property_Type_'): # Handle boolean/one-hot encoded types
                # For one-hot encoded 'Property_Type', give options related to original type
                if feature == 'Property_Type_Independent House':
                    user_inputs[feature] = st.checkbox('Is it an Independent House?', value=False)
                elif feature == 'Property_Type_Villa':
                    user_inputs[feature] = st.checkbox('Is it a Villa?', value=False)
                # 'Apartment' is the base case (both above are False)
            elif df_ml[feature].dtype == 'int64' or df_ml[feature].dtype == 'float64':
                # Use min/max from df_ml for range hints
                min_val = float(df_ml[feature].min() if not df_ml[feature].empty else 0)
                max_val = float(df_ml[feature].max() if not df_ml[feature].empty else 100)
                default_val = float(df_ml[feature].median() if not df_ml[feature].empty else (min_val + max_val) / 2)

                if df_ml[feature].dtype == 'int64':
                    user_inputs[feature] = st.number_input(f'Enter {feature.replace("_", " ")}',
                ...
                # Fallback for any other unexpected types (e.g., if any object columns somehow made it here)
                user_inputs[feature] = st.text_input(f'Enter {feature.replace("_", " ")}', '0')
    ...
    if st.button('Predict Price'):
        ...
        # Convert boolean inputs from checkboxes to int (0 or 1) to match model training
        for col in input_df.select_dtypes(include='bool').columns:
            input_df[col] = input_df[col].astype(int)

        # Ensure the order of columns in input_df matches model_features exactly
        # Reindex to ensure all features are present and in the correct order
        try:
            input_df = input_df[model_features] # Use model_features (from joblib) to reorder
        except KeyError as e:
            st.error(f"Missing feature in user input: {e}. Please ensure all required features are correctly provided.")
            st.stop()

        # Make prediction
        try:
            prediction = model.predict(input_df)[0]
            st.success(f'The predicted Future Price (in Lakhs) is: **{prediction:.2f} Lakhs**')
        except Exception as e:
            st.error(f"An error occurred during prediction: {e}")
            st.write("Please ensure all inputs are numerical and in the correct format.")

    st.info("Note: User-friendly dropdowns for categorical features would require saving the LabelEncoder/OneHotEncoder objects used during preprocessing and loading them here to map string inputs to numerical values.")
